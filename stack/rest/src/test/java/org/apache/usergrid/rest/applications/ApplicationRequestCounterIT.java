...
public class ApplicationRequestCounterIT extends AbstractRestIT {
    ...
    public void applicationrequestInternalCounters() throws Exception {
        // Get application id
        JsonNode node = mapper.valueToTree(resource().path( "/test-organization/test-app" ).queryParam( "access_token", access_token )
        ...
        // call
        node = mapper.valueToTree(resource().path( "/test-organization/test-app/counters" ).queryParam( "resolution", "all" )
